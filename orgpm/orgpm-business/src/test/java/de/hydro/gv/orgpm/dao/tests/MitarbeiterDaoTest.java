...
public class MitarbeiterDaoTest {
	...
	public void resetDatabaseTables (){
		...
		Assert.assertEquals(0, mitarbeiterDao.executeQueryWithResults("mitarbeiter.find.all").size());
	}
	...
	public void testUpdateMitarbeiter() {
		...
		Assert.assertEquals("Yevheniy", mitarbeiterDao.readAllMitarbeiter().get(0).getVorname());
